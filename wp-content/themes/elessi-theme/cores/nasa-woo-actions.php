...
/**
 * Disable default Yith Woo Compare button v3.x
 */
...
if (!function_exists('elessi_add_action_woo')) :
    function elessi_add_action_woo()
    {
        ...
        /**
         * Coupon display for Single product
         */
        if (isset($nasa_opt['single_coupon']) && $nasa_opt['single_coupon']) {
            add_action('woocommerce_single_product_summary', function () {
                global $product;
                $publish_couponts = elessi_wc_publish_coupons('single_p_coupon');
                $applicable_coupons = elessi_coupons_applicable_to_product($publish_couponts, $product);

                if (!empty($applicable_coupons)) {
                    elessi_get_coupon_slide($applicable_coupons, '1');

                    add_action('wp_footer', function () use ($product) {
?>
                        <div class="ns-alert-coupon-single nasa-flex flex-column nasa-radius-10 ns-ovhd ">
                            <span class="svg_wrap">
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-hourglass-icon lucide-hourglass">
                                    <path d="M5 22h14" />
                                    <path d="M5 2h14" />
                            ...
                            <span class="ns-alert-content text-center fs-17">
                                <?php echo sprintf(esc_html__('Adding %s to your cart with the coupon code %s', 'elessi-theme'), '<span class="nasa-bold">"' . $product->get_title() . '"</span><br>', '<span class="nasa-uppercase ns-code nasa-bold"></span>'); ?>
                            </span>
                        </div>
                <?php
                    }, 10);
        ...
        if (
            ...
            in_array(get_option('yith_woocompare_show_compare_button_in'), array('both', 'shop'))
        ) {
            add_action('nasa_show_buttons_loop', 'elessi_add_compare_in_list', 50);
        }

        /**
         * Add compare default 3.x
         */
        if (defined('YITH_WOOCOMPARE_VERSION') && version_compare(YITH_WOOCOMPARE_VERSION, '3.0.0', '>=')) {
            add_action('wp_footer', 'elessi_output_preview_bar');
            $nasa_opt['nasa-product-compare'] = '1';
            wp_enqueue_script('yith-woocompare-main');
            add_filter('yith_woocompare_remove_label', 'elessi_woocompare_remove_label');
        }
        ...
        add_action('woocommerce_before_shop_loop_item_title', 'elessi_loop_sale_flash_slide');
...
/**
 * Yith WooCommerce Compare output_preview_bar
 */
if (!function_exists('elessi_output_preview_bar')) :
    function elessi_output_preview_bar()
    {
        // array(YITH_WooCompare_Frontend::instance(), 'output_preview_bar');
        if (class_exists('YITH_WooCompare_Frontend')) {
            YITH_WooCompare_Frontend::instance()->output_preview_bar();
        }

        echo '<div class="yith-woocompare-popup-container hidden-tag"></div>';
    }
endif;

/**
 * Yith WooCommerce Compare - woocompare_remove_label
 */
if (!function_exists('elessi_woocompare_remove_label')) :
    function elessi_woocompare_remove_label($icon)
    {
        $icon = '<svg width="19" height="19" viewBox="0 0 32 32" fill="currentColor"><path d="M10.722 9.969l-0.754 0.754 5.278 5.278-5.253 5.253 0.754 0.754 5.253-5.253 5.253 5.253 0.754-0.754-5.253-5.253 5.278-5.278-0.754-0.754-5.278 5.278z" /></svg>';
            ...
            if (in_array($nasa_opt['loop_layout_buttons'], array('modern-2', 'modern-10'))) {
                ...
                if ($nasa_opt['loop_layout_buttons'] == 'modern-10') {
                    add_filter('woocommerce_get_star_rating_html', function($html, $rating, $count) {
                        ...
                        $html .= '<span class="rating hidden-tag">(' . esc_html(number_format($rating, 1)) . ')</span>';
                    ...
                    add_filter('woocommerce_product_get_rating_html', function($html, $rating, $count) {
                        ...
                        if ($rating >= 0 && !$mobile_app) {
                            $label = sprintf(__('Rated %s out of 5', 'elessi-theme'), $rating);
 ...
 * Sale Flash in loop
 */
if (!function_exists('elessi_loop_sale_flash_slide')) :
    function elessi_loop_sale_flash_slide()
    {
        global $product, $nasa_opt;

        if (!isset($nasa_opt['loop_discount_slide']) || !$nasa_opt['loop_discount_slide']) {
            return;
        }
        ...
        $svg = isset($nasa_opt['loop_discount_slide_svg']) && $nasa_opt['loop_discount_slide_svg'] != '' ? $nasa_opt['loop_discount_slide_svg'] : '<svg fill="currentColor" height="20px" width="20px" version="1.1" id="Icons" viewBox="0 0 32 32" xml:space="preserve"><g id="SVGRepo_iconCarrier"> <path d="M26.9,15.7C26.8,15.3,26.4,15,26,15h-6l3.9-11.7c0.1-0.4,0-0.9-0.4-1.1c-0.4-0.3-0.8-0.2-1.2,0l-17,13 c-0.3,0.3-0.5,0.7-0.3,1.1C5.2,16.7,5.6,17,6,17h6L8.1,28.7c-0.1,0.4,0,0.9,0.4,1.1C8.6,29.9,8.8,30,9,30c0.2,0,0.4-0.1,0.6-0.2 l17-13C26.9,16.5,27.1,16.1,26.9,15.7z"></path> </g></svg>';

        $svg = apply_filters('elessi_loop_sale_flash_svg', $svg);

        $text = isset($nasa_opt['loop_discount_slide_text']) && $nasa_opt['loop_discount_slide_text'] != '' ? $nasa_opt['loop_discount_slide_text'] : esc_html__('HOT SALE {sale} OFF.', 'elessi-theme');

        $text = apply_filters('elessi_loop_sale_flash_text', $text);
        ...
        if ($product->get_type() == 'variable') :
            ...
            foreach ($variations as $variation_data) {
                ...
                if ($variation && $variation->is_on_sale()) :
                    ...
                    if (is_numeric($sale_price)) :
                        ...
                        $id = esc_attr($variation->get_id());
                        $sale_slide_var = '<div class="nasa-custom-animate" style="display: none;" data-id="%1$s">
                        ...
                        $sale_slide_var = sprintf(
                            $sale_slide_var,
                            $id,
                            sprintf(
                                '<p class="margin-bottom-0 text-center nasa-bold"><span class="padding-right-5 padding-left-5">%1$s</span></p>',
                                str_replace('{sale}', $percentage . '%',  $text)
                            ) . $svg
                        );

                        $sale_slide .= $sale_slide_var;
                    endif;
                endif;
            ...
            if (is_numeric($sale_price)) :
                ...
                if ($percentage > 0) :
                    ...
                    $sale_slide = '<div class="nasa-custom-animate" data-id="%1$s">
                        <div class="nasa-flex infinities-slide">
                            %2$s %2$s %2$s %2$s
                        </div>
                        <div class="nasa-flex infinities-slide">
        ...
        echo $sale_slide ? '<div class="nasa-loop-discount-slide">' . $sale_slide . '</div>' : '';
    }
endif;
...
if (!function_exists('elessi_add_compare_in_list')) :
    function elessi_add_compare_in_list()
    {
         ...
         */
        if (defined('YITH_WOOCOMPARE_VERSION') && version_compare(YITH_WOOCOMPARE_VERSION, '3.0.0', '>=')) {
            if (!in_array(get_option('yith_woocompare_show_compare_button_in'), array('both', 'shop'))) {
                return;
            }
...
if (!function_exists('elessi_add_compare_in_detail')) :
    function elessi_add_compare_in_detail()
    {
         ...
         */
        if (defined('YITH_WOOCOMPARE_VERSION') && version_compare(YITH_WOOCOMPARE_VERSION, '3.0.0', '>=')) {
            if (!in_array(get_option('yith_woocompare_show_compare_button_in'), array('both', 'product'))) {
                return;
            }

            $attr = array(
                'button_or_link' => 'default',
                'button_text'    => array(
                    'is_single' => true,
                    'html' => '<span class="nasa-icon-text-wrap"><span class="margin-left-5 rtl-margin-left-0 rtl-margin-right-5 nasa-icon-text">' . esc_html__('Add to Compare', 'elessi-theme') . '</span><span class="margin-left-5 rtl-margin-left-0 rtl-margin-right-5 nasa-icon-text">' . esc_html__('Added to Compare', 'elessi-theme') . '</span></span>'
                )
            );

            YITH_WooCompare_Frontend::instance()->output_button($productId, $attr);
        }
         ...
         */
            ...
            global $yith_woocompare, $nasa_opt;
 ...
 */
if (!function_exists('elessi_nasa_change_view_layout_mobile')) :
    function elessi_nasa_change_view_layout_mobile($enable_change_view_mobile = false, $type_show = 'grid-default')
    {
        global $nasa_opt;
 ...
 */
if (!function_exists('elessi_coupons_applicable_to_product')) :
    function elessi_coupons_applicable_to_product($publish_couponts, $product)
    {
        $product_id = $product->get_id();
        $product_cats = $product->get_category_ids();
        $applicable_coupons = [];

        if ($publish_couponts && is_array($publish_couponts)) {
            foreach ($publish_couponts as $coupon) {
                ...
                $applies_to_product = ((empty($include_ids) || in_array($product_id, $include_ids)) && !in_array($product_id, $exclude_ids));

                if ($applies_to_product) {
                    if ((!empty($include_cats) && !array_intersect($product_cats, $include_cats)) ||
                        (!empty($exclude_cats) && array_intersect($product_cats, $exclude_cats))
                    ) {
        ...
        return apply_filters('elessi_applicable_coupons', $applicable_coupons, $publish_couponts, $product);
    }
endif;
