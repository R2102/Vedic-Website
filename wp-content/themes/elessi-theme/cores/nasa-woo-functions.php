...
if (!function_exists('elessi_comment_media_upload')) :
    function elessi_comment_media_upload($comment_form) {
        ...
        $label2 = '<span class="upload-media-svg">' . $cam_img . '</span>' .
            '<span class="nasa-flex flex-column align-start ns-media-text-wrap"><span class="nasa-bold-800">' . esc_html__('Upload photos', 'elessi-theme') . ' (<span class="ns-media-count">0</span>/' . $max_files .')</span>' .
...
if (!function_exists('elessi_comment_media_upload_video')) :
    function elessi_comment_media_upload_video($comment_form) {
        ...
        $label2 = '<span class="upload-media-svg">' . $cam_img . '</span>' .
            '<span class="nasa-flex flex-column align-start ns-media-text-wrap"><span class="nasa-bold-800">' . sprintf(_n('Upload video', 'Upload videos', $max_files, 'elessi-theme')) . ' (<span class="ns-media-count">0</span>/' .  $max_files . ')</span>' .
            '<span class="nasa-flie-policy">' . $svg_alert . $label . '</span>';
...
add_action( 'woocommerce_shop_loop_subcategory_title', 'elessi_woocommerce_subcategory_count_html', 10 );
if (!function_exists('elessi_woocommerce_subcategory_count_html')) :
    function elessi_woocommerce_subcategory_count_html($category) {
        ?>
		<h2 class="woocommerce-loop-category__title">
			...
			if ( $category->count > 0 ) {
				echo apply_filters( 'woocommerce_subcategory_count_html', '<span class="count">(' . $category->count . ' ' . esc_html( _n( 'item', 'items', $category->count, 'elessi-theme' ) ) .  ')</span>', $category );
			}
			?>
		</h2>
		<?php
