...
jQuery(document).ready(function($) {
    ...
    $('body').on('click', '.ns-btn-read-more', function() {
        ...
        var _wrap = $(_row).find('>.columns');
        // var _read =$(_row).find('.ns-read-more');

        var currentHeight = $(_wrap).innerHeight();
        var autoHeight = $(_wrap).css('height', 'auto').innerHeight();
        var min_height = !$(_row).hasClass('ns-tab-reviews') ? 175 : 260;
        var newHeight = (currentHeight | 0) === (autoHeight | 0) ? min_height : autoHeight;
