...
jQuery(document).ready(function ($) {
    ...
    /**
     * init read more short description 
    */
    $('body').on('init_read_more_short_description ', function () {
        if ($(".ns-read-more-short-description-wrap").length) {
            $('.woocommerce-product-details__short-description').each(function(){
                var wrap = $(this).find(".ns-read-more-short-description-wrap");
                var content_Height = $(this).find(".ns-short-description-wrap").innerHeight();
                var wrap_Height = $(wrap).innerHeight();
                if (wrap_Height >= content_Height && $(this).find('.ns-btn-read-more-sdesc').length) {
                    $(this).find('.ns-btn-read-more-sdesc').remove();
                    $(wrap).css('height', 'auto');
                }
            });
        }
    }).trigger('init_read_more_short_description');
    ...
    $(window).on('scroll', function () {
        ...
        if (
            ...
            if (scrollTop > lastScrollTop) {
                clearTimeout(removeHeaderStyleTimeout);
                if ($('#header-content').hasClass('nasa-header-no-collapse')) {
            ...
            if ((scrollTop + screen_h >= documentHeight - _heightFixed * 3) && $('body').hasClass('nasa-iphone-detected')) {
                time = 500;
            }
            ...
            if (scrollTop < lastScrollTop || scrollTop + screen_h >= documentHeight) {
                ...
                clearTimeout(removeHeaderStyleTimeout);
                removeHeaderStyleTimeout = setTimeout(function () {
    ...
    $('body').find('input.qty:not(.product-quantity input.qty)').each(function () {
        ...
        var min = ns_round_2_step($(this).attr('min'), _step);
        if (min && min > 0 && ns_round_2_step($(this).val(), _step) < min) {
            $(this).val(min);
        }
    });
    ...
    $('body').on('click', '.plus, .minus', function () {
        ...
        var $qty = $(this).parents('.quantity').find('.qty'),
            ...
            currentVal = ns_round_2_step($qty.val(), step),
            max = ns_round_2_step($qty.attr('max'), step),
            min = ns_round_2_step($qty.attr('min'), step);
        ...
        if ($(this).hasClass('plus')) {
            if (max && (max == currentVal || currentVal > max)) {
                ...
                $qty.val(ns_round_2_step(currentVal + parseFloat(step), step));
                if (button_add && button_add.length) {
                    button_add.attr('data-quantity', ns_round_2_step(currentVal + parseFloat(step), step));
                }
            }
        }
    ...
    $('body').on('click', '.ns-btn-read-more-sdesc', function () {
        var _this = $(this);
        var _pa = $(_this).parents('.woocommerce-product-details__short-description');
        var wrap = $(_pa).find(".ns-read-more-short-description-wrap");
        var currentHeight = $(wrap).innerHeight();
        var autoHeight = $(wrap).css('height', 'auto').innerHeight();
        var newHeight = (currentHeight | 0) === (autoHeight | 0) ? $(wrap).attr('data-height') : autoHeight;

        if (newHeight == autoHeight) {
            $(_this).find('.ns-btn-read-more-text').text($(_this).attr('data-show-less'));
            $(_this).addClass('ns-active');
        } else {
            $(_this).find('.ns-btn-read-more-text').text($(_this).attr('data-show-more'));
            $(_this).removeClass('ns-active');
        }

        $(wrap).css('height', currentHeight).animate({
            height: (newHeight)
        }, function () {
            $(window).scroll();
        });
