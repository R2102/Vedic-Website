...
var notificationInterval, _nasa_clear_notice_coupon;
...
jQuery(document).ready(function ($) {
    ...
    if ($('body').hasClass('nasa-has-yth-compare-3_0')) {
        $(document).on('yith_woocompare_product_added yith_woocompare_product_removed', function (e, data) {
            if ($('.nasa-close-notice').length) {
                $('.nasa-close-notice').trigger('click');
            }

            var btn = $('.nasa-compare[data-prod="' + data.productId + '"]');

            if ($(btn).find('.nasa-icon-text-wrap').length) {
                if ($(btn).hasClass('added')) {
                    $(btn).find(".nasa-icon-text-wrap").animate({ scrollTop: 24 }, 400);
                } else {
                    $(btn).find('.nasa-icon-text-wrap').animate({ scrollTop: 0 }, 400);
                }
            }

            if (!$('#yith-woocompare').length) {
                show_compare($);
            }

            if ($('#yith-woocompare-preview-bar').length) {
                var count_compare = $('#yith-woocompare-preview-bar').attr('data-count-compare');
                if ($('.nasa-mini-number.compare-number').length) {
                    $('.nasa-mini-number.compare-number').html(convert_count_items($, count_compare));
                    if (count_compare == 0) {
        ...
        $(document).on('yith_woocompare_fragments_replaced', function () {
            if ($('body').hasClass('yith-woocompare-popup-open')) {
                show_compare($);
    ...
    $('body').on('click', '.nasa-show-compare', function () {
        ...
        if (!$(this).hasClass('nasa-showed') || !$(compareList).hasClass('nasa-active')) {
            show_compare($);
        } else {
            hide_compare($);
        }
    ...
    $('body').on('click', '.btn-wishlist', function () {
        ...
        if (!$(_this).hasClass('nasa-disabled')) {
             ...
             */
                ...
                var _button = $(_this).parent();
                if ($('body').hasClass('nasa-has-yith-wcwl-4_5')) {
                    var _wl_inited = $(_button).find('.yith-add-to-wishlist-button-block--initialized');

                    if (!$(_this).hasClass('nasa-added') && $(_wl_inited).length && $(_wl_inited).find('.yith-wcwl-add-to-wishlist-button').length) {
                        $(_this).addClass('nasa-added');

                        $(_wl_inited).find('.yith-wcwl-add-to-wishlist-button').get(0).click();

                        if ($('.ns-mini-yith-wcwl .nasa-wishlist-count').length) {
                            var _count_wl = parseInt($('.ns-mini-yith-wcwl .nasa-wishlist-count').text());
                            _count_wl = _count_wl ? _count_wl + 1 : 1;

                            $('.ns-mini-yith-wcwl .nasa-wishlist-count').html(_count_wl);

                            if (_count_wl <= 0) {
                                $('.ns-mini-yith-wcwl .nasa-wishlist-count').addClass('nasa-product-empty');
                            } else {
                                $('.ns-mini-yith-wcwl .nasa-wishlist-count').removeClass('nasa-product-empty');
                            }
                        }
                    }
    ...
    */
    ...
    $('body').on('get_content_popup_v2', function (e, reload_cross) {
        if ($('.ns-cart-popup-wrap .ns-cart-popup-v2').length) {
            ...
            $('#cart-sidebar .woocommerce-mini-cart-item:not(.woosb-cart-child)').each(function () {
    ...
    $('body').on('adding_to_cart', function (e, $thisbutton, data) {
        ...
        if ($('.yith-wceop-cart-widget').length <= 0) {
            ...
            if ($('input[name="ns-not-preload-mcart"]').length <= 0) {
                if ((_event_add === 'sidebar' || is_update_mini_cart) && $('form.nasa-shopping-cart-form, form.woocommerce-checkout').length <= 0) {
                    if ($('#cart-sidebar .cross-sells').length) {
                        $('body').trigger('nasa_cross_sell_columns_setting', [$('#cart-sidebar .cross-sells').find('.nasa-slick-slider')]);
                    }
    ...
    $('body').on('click', '.toggle-sidebar-shop', function () {
        var _this = $(this);
        var _pa = $(_this).parents('.nasa-toggle-layout-side-sidebar');
    ...
    $('body').on('nasa_media_all_pause', function () {
        ...
        $('iframe').each(function () {
            ...
            if ($(iframe).attr('title') == 'reCAPTCHA') {
                return; // Skip reCAPTCHA iframes
            }

            if (src && src.includes('youtube.com/embed') && iframe.contentWindow) {
                iframe.contentWindow.postMessage('{"event":"command","func":"pauseVideo","args":""}', '*');
            } else if (src && src.includes('vimeo.com') && iframe.contentWindow) {
