...
class cmb_Meta_Box {
     ...
     */
    public static function show_form($meta_box = array(), $object_id = 0, $object_type = '') {
     ...
     */
    public function save_post($post_id = 0, $post = false) {
     ...
     * @param  string $object_type Type of object being saved. (e.g., post, user, or comment)
     */
    public static function save_fields($_meta_box = array(), $object_id = 0, $object_type = '') {
        $meta_box = self::set_mb_defaults($_meta_box);
    ...
    public static function sanitize_field($field = null, $new_value = null) {
    ...
    public static function remove_option($option_key, $field_id) {
        ...
        self::$options[$option_key] = (!isset(self::$options[$option_key]) || empty(self::$options[$option_key])) ?
            self::_get_option($option_key) : self::$options[$option_key];
     ...
     * @return array               Modified options
     */
    public static function update_option($option_key = '', $field_id = '', $value = null, $single = true) {
     ...
     */
    public static function image_id_from_url($img_src) {
        global $wpdb;

        $img_url = esc_url_raw($img_src);
 ...
 */
function cmb_get_field($field_args = array(), $object_id = 0, $object_type = 'post') {
    // Default to the loop post ID
    // $id = $object_id ? $object_id : get_the_ID();
    cmb_Meta_Box::set_object_id($object_id ? $object_id : get_the_ID());
 ...
 */
function cmb_get_field_value($field_args = array(), $object_id = 0, $object_type = 'post') {
 ...
 */
function cmb_metabox_form($meta_box = array(), $object_id = 0, $echo = true) {
