...
class Nasa_WC_Attr_UX_Admin extends Nasa_Abstract_WC_Attr_UX {
    ...
    public function __construct() {
        ...
        add_action('nasa_attr_ux_product_attribute_field', array($this, 'attribute_fields'), 10, 4);
    ...
    public function add_attribute_fields($taxonomy) {
        ...
        do_action('nasa_attr_ux_product_attribute_field', $attr->attribute_type, '', 'add', array());
    }
    ...
    public function edit_attribute_fields($term, $taxonomy) {
        ...
        do_action('nasa_attr_ux_product_attribute_field', $attr->attribute_type, $value, 'edit', $term);
    }
     ...
     * @param $form
     */
    public function attribute_fields($type, $value, $form, $term) {
        global $nasa_opt;
        ...
        switch ($type) {
            case self::_NASA_IMAGE :
                ...
                <div id="ns-attr-image_thumbnail" style="float: left; margin-right: 10px;"><?php echo $image; ?></div>
                <div style="line-height: 60px;">
                    ...
                    <button data-no_img="<?php echo esc_url(self::$no_image); ?>" type="button" class="remove_image-tax button hidden-tag"<?php echo $style; ?>><?php echo esc_html_e('Remove Image', 'nasa-core'); ?></button>
                </div>
                <?php
                break;
            
            case self::_NASA_COLOR :
                $colors = $value ? explode(',', $value) : array();
                $colors = apply_filters('ns_admin_attr_color_arr', $colors);
                ?>
                <div class="colors-wrap nasa-flex flex-column align-start">
                    <div class="main-color-wrap ns-color-item">
                        <span class="ns-label"><?php echo esc_html__('Color 1st', 'nasa-core'); ?></span>
                        <input type="text" id="term-<?php echo esc_attr($type); ?>-1st" class="ns-color-picker" value="<?php echo isset($colors[0]) ? esc_attr($colors[0]) : ''; ?>" />
                    </div>
                    
                    <div class="second-color-wrap ns-color-item">
                        <span class="ns-label"><?php echo esc_html__('Color 2nd', 'nasa-core'); ?></span>
                        <input type="text" id="term-<?php echo esc_attr($type); ?>-2nd" class="ns-color-picker" value="<?php echo isset($colors[1]) ? esc_attr($colors[1]) : ''; ?>" />
                    </div>
                    
                    <input id="ns-values-colors" type="hidden" name="<?php echo esc_attr($type); ?>" value="<?php echo esc_attr($value); ?>" />
                </div>
                <?php 
                break;
        ...
        switch ($type) {
            ...
            case self::_NASA_COLOR :
                if (empty($term)  || !isset($nasa_opt['attribute_single_fomo']) || !$nasa_opt['attribute_single_fomo']) {
                    break;
                }

                $this->_render_field('nasa_fomo', __('Fomo label', 'nasa-core'), '', $term, $is_edit);
                $this->_render_field('nasa_fomo_bg', __('Fomo label background', 'nasa-core'), 'ns-color-picker', $term, $is_edit);
                $this->_render_field('nasa_fomo_color', __('Fomo label color', 'nasa-core'), 'ns-color-picker', $term, $is_edit);
     ...
     * @param type $is_edit
     */
    protected function _render_field($id, $label, $class, $term, $is_edit) {
        $value = get_term_meta($term->term_id, $id, true);
        $tag   = $is_edit ? 'tr' : 'div';
        $head  = $is_edit ? '<th><label for="term-' . esc_attr($id) . '">' . esc_html($label) . '</label></th><td>' : '<label for="term-' . esc_attr($id) . '">' . esc_html($label) . '</label>';
        $foot  = $is_edit ? '</td></tr>' : '</div>';

        echo '<' . $tag . ' class="form-field">' . $head . '<input type="text" id="term-' . esc_attr($id) . '" name="' . esc_attr($id) . '" value="' . esc_attr($value) . '"' . ($class ? ' class="' . esc_attr($class) . '"' : '') . ' />' . $foot;
    }
    ...
    public function save_term_meta($term_id, $tt_id) {
        ...
        if (isset($_POST['nasa_fomo'])) {
            update_term_meta($term_id, 'nasa_fomo', $_POST['nasa_fomo']);
        }

        if (isset($_POST['nasa_fomo_bg'])) {
            update_term_meta($term_id, 'nasa_fomo_bg', $_POST['nasa_fomo_bg']);
        }

        if (isset($_POST['nasa_fomo_color'])) {
            update_term_meta($term_id, 'nasa_fomo_color', $_POST['nasa_fomo_color']);
        }
    }
    ...
    public function add_attribute_column_content($columns, $column, $term_id) {
        ...
        switch ($attr->attribute_type) {
            case self::_NASA_COLOR:
                ...
                echo '<div class="nasa-attr-ux-preview nasa-attr-ux-color nasa-flex">';
                
                if (isset($colors[0])) {
                    echo '<div class="color-item-1st color-item" style="background-color: ' . esc_attr($colors[0]) . ';"></div>';
                }
                
                if (isset($colors[1])) {
                    echo '<div class="color-item-2nd color-item" style="background-color: ' . esc_attr($colors[1]) . ';"></div>';
                }
