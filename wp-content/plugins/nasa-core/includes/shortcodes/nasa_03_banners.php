...
function nasa_sc_banners($atts = array(), $content = null) {
    ...
    $ratio = isset($image[2]) && $image[2] ? $a['height'] / $image[2] : false;
    $a['width'] = !(int) $a['width'] ? (isset($image[1]) && $image[1] && $ratio ? ($ratio * $image[1]) : 200) : (int) $a['width'];
    
    $height = 'height: ' . (int) $a['height'] . 'px;';
    $text_align = ($a['text-align'] != '') ? ' ' . $a['text-align'] : '';
    $hover_effect = ($a['hover'] != '') ? ' hover-' . $a['hover'] : '';
    $content_width = ($a['content-width'] != '') ? 'width: ' . esc_attr($a['content-width']) . ';' : '';
    ...
    $content = trim($content) ?
        '<div class="banner-content-warper"><div class="nasa-banner-content banner-content' . esc_attr($a_class . $text_align) . '" style="' . $content_width . $move_x . '">' .
            '<div class="banner-inner nasa-transition wow ' . esc_attr($effect_text . $class_woo) . '" data-animation="' . esc_attr($effect_text) . '">' . 
    ...
    return 
    '<div class="banner nasa-banner' . esc_attr($hover_effect . $el_class) . '" data-wow-delay="' . esc_attr($data_delay) . '"' . $onclick . ' style="' . $height . '">' .
        '<div class="banner-image nasa-banner-image ns-banner-bg" style="' . $banner_bg . '" data-height="' . esc_attr($a['height']) . '" data-width="' . esc_attr($a['width']) . '"></div>' .
        $content .
    '</div>';
}
