...
function nasa_sc_product_nasa_categories($atts = array(), $content = null) {
    ...
    $class_form .= ' nasa-style-' . esc_attr($style);
    $class_form .= $el_class ? ' ' . esc_attr($el_class) : '';
    ...
    $content = '<form class="' . $class_form . '" method="get" data-hide_empty="' . $hideEmpty . '" data-show_count="' . $count_items . '"' . $action_form . '>' .
        '<div class="row"><div class="' . esc_attr($class_wrap_all) . ' nasa-selector nasa-filter-custom-tax"><div class="row">';
    ...
    if ($arrayCats) {
        for ($n = $k; $n >= 0; $n--) {
            ...
            $content .= '<div class="' . esc_attr($class_wrap) . ' nasa-wrap-select" data-active="' . (isset($arrayCatsAct[$n]) ? $arrayCatsAct[$n] : '') . '">';
            $content .= '<select data-key="' . $level . '" data-target=".nasa-select-' . ($level + 1) . '" class="nasa-select2 nasa-filter-nasa-categories nasa-select-' . $level . '" data-text_select="' . esc_attr($emptySelect) . '">';
            $content .= '<option value="">' . esc_html($emptySelect) . '</option>';
            ...
            foreach ($currents as $item) {
                ...
                $content .= '<option value="' . esc_attr($item->slug) . '"' . $active . '>' . esc_html($label) . '</option>';
            }
    ...
    if ($level < $max_level) {
        for ($n = $level; $n < $max_level; $n++) {
            ...
            $content .= '<div class="' . $class_wrap . ' nasa-wrap-select">' .
                '<select data-key="' . $n . '" data-target=".nasa-select-' . ($n + 1) . '" class="nasa-select2 nasa-filter-nasa-categories nasa-select-' . $n . '" data-text_select="' . esc_attr($emptySelect) . '">';
            $content .= '<option value="">' . esc_html($emptySelect) . '</option>';
            $content .= '</select></div>';
        }
    }
     ...
     * Input name
     */
    $content .= '<input class="nasa-input-main nasa-custom-cat" type="hidden" name="' . esc_attr($nasa_taxonomy) . '" value="' . esc_attr($active_slug) . '" />';
...
function nasa_register_product_nasa_categories(){
    vc_map(array(
        ...
        'description' => __("Filter products by Product Groups.", 'nasa-core'),
        "class" => "",
        "category" => 'Nasa Core',
        "params" => array(
            array(
            ...
            array(
                "type" => "dropdown",
                "heading" => __("Show Count products", 'nasa-core'),
            ...
            array(
                "type" => "dropdown",
                "heading" => __("Deep Levels", 'nasa-core'),
            ...
            array(
                "type" => "textfield",
                "heading" => __("Filter Text", 'nasa-core'),
                "param_name" => "button_text"
            ),
            
            array(
                "type" => "dropdown",
                "heading" => __("Submit Redirect To", 'nasa-core'),
