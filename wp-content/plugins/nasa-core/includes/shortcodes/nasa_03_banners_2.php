...
function nasa_sc_banners_2($atts = array(), $content = null) {
    ...
    $class_wrap .= $hover != '' ? ' hover-' . esc_attr($hover) : '';
    $class_wrap .= $el_class != '' ? ' ' . esc_attr($el_class) : '';
    
    $data_attrs = $data_delay != '' ? ' data-wow-delay="' . esc_attr($data_delay) . '"' : '';
    if ($link != '') {
    ...
    if ($content_width != '') {
        $ct_attrs[] = 'width: ' . esc_attr($content_width);
    }
    
    if ($move_x != '') {
        if ($align == 'left') {
            $ct_attrs[] = 'left: ' . esc_attr($move_x);
        }
        
        if ($align == 'right') {
            $ct_attrs[] = 'right: ' . esc_attr($move_x);
        }
    }
    ...
    $content_data .= trim($content) ?
        '<div class="' . esc_attr($a_class) . '"' . $ct_attrs_str . '>' .
