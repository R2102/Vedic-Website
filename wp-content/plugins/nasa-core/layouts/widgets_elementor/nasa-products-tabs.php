 ...
 * Build array tabs
 */
$tabs_heading = $tabs_content = $before_title = $after_title = $active_image = $inactive_image = array();
...
foreach ($instance['tabs'] as $key => $tab) :
     ...
     */
    $active_image[$key] = '';
    if (isset($tab['tab_active_image'])) :
        $active_image[$key] = $tab['tab_active_image'];
        unset($tab['tab_active_image']);
    endif;
     ...
     */
    $inactive_image[$key] = '';
    if (isset($tab['tab_inactive_image'])) :
        $inactive_image[$key] = $tab['tab_inactive_image'];
        unset($tab['tab_inactive_image']);
    endif;
...
foreach ($tabs_heading as $k => $heading) :
    ...
    // Add image display if images are provided
    if (!empty($active_image[$k]) || !empty($inactive_image[$k])) :
        echo '<span class="nasa-tab-image-wrap">';
        
        // Active image
        if (!empty($active_image[$k])) :
            $active_img_src = wp_get_attachment_image_src($active_image[$k], 'thumbnail');
            if ($active_img_src) :
                echo '<img src="' . esc_url($active_img_src[0]) . '" class="nasa-tab-active-image' . ($stt == 1 ? ' active' : '') . '" alt="' . esc_attr($heading) . '" style="max-width: 40px; height: auto;" />';
            endif;
        endif;
