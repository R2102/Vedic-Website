...
echo '<div class="nasa-intagram-wrap' . ($el_class != '' ? ' ' . esc_attr($el_class) : '') . '" data-layout="slider" data-size="' . esc_attr($width) . '">';
...
for ($i = 0; $i < $limit_items; $i++) :
    echo '<a href="#" target="_blank" class="' . esc_attr($aclass) . '" rel="nofollow" data-index="' . $i . '">';
        echo '<img src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8Xw8AAoMBgDTD2qgAAAAASUVORK5CYII=" class="' . esc_attr($imgclass) . '" alt="Instagram Image" width="' . esc_attr($width) . '" height="' . esc_attr($height) . '" />';
    echo '</a>';
endfor;
echo '</div>';
